...
const Manual = () => {
  ...
  return (
    <main className="flex flex-col bg-[#0A0A0A] max-h-full h-screen w-screen">
      ...
      <div className="flex px-[100px] flex-1 min-h-0">
        <div className="flex flex-col border-r border-zinc-800 py-[20px] overflow-y-auto">
          {titles.map((x, index) => {
            return <button key={index} className={`shrink-0 cursor-pointer text-start w-[260px] h-[40px] px-5 border-l-3 border-transparent  ${index === selected ? 'bg-zinc-800 text-white border-l-brand' : 'text-zinc-500 hover:bg-zinc-900/70'}`} onClick={() => scrollToHeading(index)}>{x.text}</button>
          })}
        </div>
